...
class RailGuard:
    def __init__(self) -> None:
        ...
        railguard_prompt = """
You are a railguard meant to judge if the user's question is strictly related to F-1 visa regulations, CPT, or OPT. 
You may receive follow-up questions that refer to a previous question asked in the chat. In such cases, consider the follow-up as relevant if it is contextually connected to the previous conversation, even if the question does not explicitly mention F-1 visa details.
Specifically, determine if the question pertains only to rules, regulations, or instructions for international students studying in the United States on an F-1 visa.
If the question is directly about F-1 visa regulations, CPT, or OPT, reply True.
If the question includes tangential topics (e.g., H-1B visa, Green Card, job sponsorship), reply False.
...
#         prompt = (
#             f"{self.reasoning_prompt}\n\n"
#             f"Use this context to answer if needed:{context_str}\n\n"
#             f"Question: {query_text}\n\n"
#         )
